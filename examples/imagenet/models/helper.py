...
class DataSpec(object):
  ...
  def __init__(self,
    ...
    # The mean to be subtracted from each image. By default, the per-channel
    # ImageNet mean.
    # The values below are ordered BGR, as many Caffe models are trained in
    # this order.
    # Some of the earlier models (like AlexNet) used a spatial
    # three-channeled mean.
    # However, using just the per-channel mean values instead doesn't affect
    # things too much.
...
def alexnet_spec(batch_size=500):
  ...
  return DataSpec(batch_size=batch_size, scale_size=256, crop_size=227,
                  isotropic=False)


def std_spec(batch_size, isotropic=True):
  '''Parameters commonly used by "post-AlexNet" architectures.'''
  return DataSpec(batch_size=batch_size, scale_size=256, crop_size=224,
                  isotropic=isotropic)


# Collection of sample auto-generated models
MODELS = (
AlexNet, CaffeNet, GoogleNet, NiN, ResNet50, ResNet101, ResNet152, VGG16)
