...
def process_image(img, scale, isotropic, crop, mean):
  ...
  # Center crop
  # Use the slice workaround until crop_to_bounding_box supports deferred
  # tensor shapes
  # See: https://github.com/tensorflow/tensorflow/issues/521
  offset = (new_shape - crop) / 2
  img = tf.slice(img, begin=tf.pack([offset[0], offset[1], 0]),
...
class ImageProducer(object):
  ...
  def __init__(self, image_paths, data_spec, num_concurrent=4, batch_size=None,
               labels=None):
  ...
  def setup(self, batch_size, num_concurrent):
    ...
    if num_images % batch_size != 0:
      raise ValueError(
              'The total number of images ({}) must be divisible by the batch '
              'size ({}).'.format(
    ...
    self.enqueue_paths_op = self.path_queue.enqueue_many(
            [indices, self.extension_mask,
             self.image_paths])
    # Close the path queue (no more additions)
    self.close_path_queue_op = self.path_queue.close()

    # Create an operation that dequeues a single path and returns a processed
    #  image
    (idx, processed_image) = self.process()

    # Create a queue that will contain the processed images (and their indices)
    image_shape = (
    self.data_spec.crop_size, self.data_spec.crop_size, self.data_spec.channels)
    processed_queue = tf.FIFOQueue(
      capacity=int(np.ceil(num_images / float(num_concurrent))),
  ...
  def start(self, session, coordinator, num_concurrent=4):
    ...
    return self.queue_runner.create_threads(session, coord=coordinator,
                                            start=True)

  def get(self, session):
    '''
    Get a single batch of images along with their indices. If a set of labels
    were provided,
  ...
  def load_image(self, image_path, is_jpeg):
    ...
    img = tf.cond(
            is_jpeg,
            lambda: tf.image.decode_jpeg(file_data,
                                         channels=self.data_spec.channels),
            lambda: tf.image.decode_png(file_data,
                                        channels=self.data_spec.channels))
...
class ImageNetProducer(ImageProducer):
  def __init__(self, val_path, data_path, data_spec):
    ...
    # The get_ilsvrc_aux.sh in Caffe's data/ilsvrc12 folder can fetch a copy
    # of val.txt
