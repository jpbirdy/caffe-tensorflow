...
def get_padding_type(kernel_params, input_shape, output_shape):
  ...
  https://github.com/Yangqing/caffe2/blob/master/caffe2/proto/caffe2_legacy
  .proto
  '''
...
class TensorFlowMapper(NodeMapper):
  ...
  def map_convolution(self, node):
    ...
    return MaybeActivated(node)('conv', kernel_params.kernel_h,
                                kernel_params.kernel_w, c_o,
...
class TensorFlowTransformer(object):
  ...
  def load(self, def_path, data_path, phase):
    ...
    transformers = [
      ...
      ReLUFuser(
        allowed_parent_types=[NodeKind.Convolution, NodeKind.InnerProduct,
                              NodeKind.BatchNorm]),
  ...
  def transform_data(self):
    if self.params is None:
      ...
      self.params = {node.name: node.data for node in self.graph.nodes if
                     node.data}
    return self.params
