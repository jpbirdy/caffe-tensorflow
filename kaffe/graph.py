...
class Graph(object):
  ...
  def __str__(self):
    ...
    for node in self.topologically_sorted():
      ...
      s.append(
        '{:<20} {:<30} {:>20} {:>20}'.format(node.kind, node.name, data_shape,
                                             tuple(out_shape)))
    return '\n'.join(s)
...
class GraphBuilder(object):
  ...
  def build(self):
    ...
    # In Caffe, each layer can produce multiple outputs ("tops") from a set
    # of inputs
    # ("bottoms"). The bottoms refer to other layers' tops. The top can
    # rewrite a bottom
    # (in case of in-place operations). Note that the layer's name is not
    # used for establishing
    # any connectivity. It's only used for data association. By convention,
    # a layer with a
    # single top will often use the same name (although this is not required).
    #
    # The current implementation only supports single-output nodes (note that
    #  a node can still
    ...
    for layer in layers:
      ...
      for output_name in layer.top:
        ...
        # Since we are working in the single-output regime, we will can
        # rename it to
