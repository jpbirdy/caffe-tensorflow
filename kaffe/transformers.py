...
A transformer is a callable that accepts a graph and returns a transformed
version.
'''
...
class DataInjector(object):
  '''
  Associates parameters loaded from a .caffemodel file with their
  corresponding nodes.
  '''
  ...
  def adjust_parameters(self, node, data):
    if not self.did_use_pb:
      return data
    # When using the protobuf-backend, each parameter initially has four
    # dimensions.
    # In certain cases (like FC layers), we want to eliminate the singleton
    # dimensions.
    # This implementation takes care of the common cases. However, it does
    # leave the
    ...
    for node in graph.nodes:
      ...
      if (node.kind == NodeKind.InnerProduct) and self.has_spatial_parent(node):
        ...
        weights = weights.reshape(
                (output_channels, in_shape.channels, in_shape.height,
                 in_shape.width))
...
class ReLUFuser(SubNodeFuser):
  ...
  def is_eligible_pair(self, parent, child):
    return ((
            self.allowed_parent_types is None or parent.kind in
            self.allowed_parent_types) and
...
class BatchNormScaleBiasFuser(SubNodeFuser):
  ...
  def is_eligible_pair(self, parent, child):
    return (
    parent.kind == NodeKind.BatchNorm and child.kind == NodeKind.Scale and
...
class ParameterNamer(object):
  '''
  Convert layer data arrays to a dictionary mapping parameter names to their
  values.
  '''
